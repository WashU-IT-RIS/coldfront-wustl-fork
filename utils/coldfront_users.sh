...
kubectl exec $COLDFRONT_CONTAINER -n coldfront -- /bin/bash -c "PYTHONPATH=/tmp USER_LIST_PI_ONLY=True USER_LIST_RESOURCE=${STORAGE_RESOURCE} coldfront shell < /tmp/user_list.py"
