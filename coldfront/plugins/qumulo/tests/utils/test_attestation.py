...
def _completed_record(**overrides):
    record = {
        "universal_id": 12345,
        "attestation_cycle_id": "att_2026_q3",
        "due_date": "2026-09-01",
    }
    ...
    def test_attestation_gate_enabled_reads_truthy_values(self):
        for value in ["1", "true", "True", "yes"]:
            with patch.dict("os.environ", {"ATTESTATION_GATE_ENABLED": value}):
                self.assertTrue(
                    attestation_gate_enabled(), f"expected {value!r} to enable the gate"
                )
...
class TestFindCompletedAttestation(TestCase):
    def test_matches_resolved_wustlkey(self):
        ...
        record = find_completed_attestation(
            "sleong", mock_ad_api, [_completed_record()]
        )
    ...
    def test_returns_none_when_user_not_in_completed_list(self):
        ...
        self.assertIsNone(
            find_completed_attestation("sleong", mock_ad_api, [_completed_record()])
        )
    ...
    def test_skips_unresolvable_records(self):
        records = [
            _completed_record(universal_id=11111),
            _completed_record(universal_id=22222),
        ]
