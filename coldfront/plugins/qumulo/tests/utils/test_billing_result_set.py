...
from coldfront.core.allocation.models import (
    AllocationAttributeType, AllocationStatusChoice, AllocationAttribute, AllocationAttributeUsage
)
...
class TestBillingResultSet(TestCase):
    def setUp(self):
        ...
        storage_quota = self.new_alloc.allocationattribute_set.get(allocation_attribute_type__name="storage_quota")
        [
        ...
        self.out_of_date_alloc.status = AllocationStatusChoice.objects.get(name="Active")
    ...
    def test_monthly_billing_cycle_result_set(self):
        listl = BillingResultSet.retrieve_billing_result_set("monthly", "2025-05-01", "2025-05-31")
        count = len([l for l in listl if isinstance(l, dict)])
        expected_usage = AllocationAttributeUsage.history.filter(allocation_attribute__allocation=self.new_alloc, 
                                                        allocation_attribute__allocation_attribute_type__name="storage_quota",
                                                        history_date__date="2025-05-31").values("value")
        expected_dict = {'billing_cycle': 'monthly', 'cost_center': 'Uncle Pennybags', 'subsidized': 'No', 'billing_exempt': 'No', 'pi': 'test', 'usage': expected_usage[0]["value"]}
