...
class StorageUsageReport:
    ...
    def _format_usage_report(self, usages: list) -> str:
        ...
        for key in keys:
            ...
            is_float_col[key] = all(self._is_float(entry[key]) for entry in usages)
    ...
    def get_allocation_usage_kb_by_date(self, allocation: Allocation, usage_date: date) -> float:
        usage = AllocationAttributeUsage.history.filter(
        ...
        ).values("value").first().get("value", 0.0) / 1024  # Convert to KB
        return float(usage)
    ...
    def get_usages_by_pi_for_school(self, school: str) -> list:
        ...
        for allocation in allocations:
            ...
            usage = self.get_allocation_usage_kb_by_date(allocation, self.usage_date)
