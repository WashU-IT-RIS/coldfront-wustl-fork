...
def get_billing_query(args: dict, report_type: str) -> str:

    fiscal_year = _get_current_fiscal_year()
    if report_type == "monthly":
        report_header_type = "Monthly for"
    elif report_type == "prepaid":
        report_header_type = "Prepaid for"
    if report_type == "monthly":
        item_description = _get_item_description(MONTHLY_COLS)
    elif report_type == "prepaid":
        item_description = _get_item_description(PREPAID_COLS)
    ...
    if report_type == "prepaid":
        ...
        prepaid_custom_columns_from_select_lowest_level = "prepaid_billing_date,\nprepaid_expiration,\n            prepaid_time,"
    ...
    if report_type == "monthly":
        ...
        final_where_clause = f"WHERE prepaid_billing_date = '{args['delivery_date']}'"
