...
def __set_daily_quota_usages(all_quotas, logger) -> None:
    ...
    for quota in all_quotas["quotas"]:
        allocation = __get_allocation_by_attribute(
            storage_filesystem_path_attribute_type, quota["path"]
        )
        if allocation is None:
            continue
        allocation.set_usage("storage_quota", quota["capacity_usage"])
...
def __validate_results(quota_usages, logger) -> bool:
    ...
    if success:
        logger.info("Successful ingestion of quota daily usage.")
    else:
        logger.info("Unsuccessful ingestion of quota daily usage. Check the results.")
