...
from coldfront_plugin_qumulo.fields import ADUserField
...
class AllocationForm(forms.Form):
    ...
    technical_contact = forms.CharField(
        help_text="Who should be contacted regarding technical details",
        label="Technical Contact",
        validators=[validate_single_ad_user],
        required=False,
    )
    billing_contact = forms.CharField(
        help_text="Who should be contacted regarding billing details",
    ...
    storage_filesystem_path = forms.CharField(
        ...
        validators=[validate_leading_forward_slash, validate_parent_directory],
    )
    ...
    def clean(self) -> dict[str, Any]:
        ...
        if "s3" in protocols:
            self._validate_s3_allocation_name(cleaned_data.get("storage_name", ""))
    ...
    def get_project_choices(self) -> list[str]:
        ...
        if (
            user.is_superuser
            or user.has_perm('project.can_view_all_projects')
        ):
