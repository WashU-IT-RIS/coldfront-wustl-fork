...
class TestUpdateStorageAllocationPendingStatus(TestCase):
    ...
    def test_conditionally_update_storage_allocation__status_sets_status_to_new_on_success(
        self,
    ) -> None:
    ...
    def test_conditionally_update_storage_allocation_status_does_nothing_when__acls_are_pending(
        self,
    ) -> None:
        allocation = create_allocation(
            project=self.project, user=self.user, form_data=self.form_data
