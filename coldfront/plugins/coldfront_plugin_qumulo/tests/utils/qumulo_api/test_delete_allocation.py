...
class DeleteAllocation(TestCase):
    ...
    def test_deletes_s3_bucket(self, mock_RestClient: MagicMock):
        with patch.object(
            ...
            qumulo_instance = QumuloAPI()
            qumulo_instance._delete_allocation(
                fs_path="/foo", export_path="bar", name="foo", protocols=["s3"]
            )

            mock_RestClient.assert_called_once()
            mock_delete_protocol.assert_called_once_with(
                export_path="bar", name="foo", protocol="s3"
            )
    ...
    def test_deletes_multiple_allocations(self, mock_RestClient: MagicMock):
        with patch.object(
            ...
            calls = [
                ...
                call(export_path="bar", name="foo", protocol="s3"),
    ...
    def test_deletes_s3_bucket_with_only_required_params(
        ...
        with patch.object(
            ...
            qumulo_instance = QumuloAPI()
            qumulo_instance._delete_allocation(
                fs_path="/foo", name="foo", protocols=["s3"]
            )

            mock_RestClient.assert_called_once()
            mock_delete_protocol.assert_called_with(
                export_path=None, name="foo", protocol="s3"
            )
