import arrow
from django.core.management.base import BaseCommand
from django.core.mail import send_mail
from django_q.tasks import schedule
from django_q.models import Schedule
from datetime import datetime, timezone
from coldfront.plugins.integratedbilling.storage_usage_report import StorageUsageReport

SCHEDULED_FOR_2ND_DAY_OF_MONTH_AT_6_AM = (
    arrow.utcnow().replace(day=2, hour=6, minute=00).format(arrow.FORMAT_RFC3339)
)


class Command(BaseCommand):

    def add_arguments(self, parser):
        parser.add_argument(
            "--tier",
            type=str,
            default="1",
            help="Service Tier(default: 1, options: Active-- 1, Archive-- 2)",
        )
        parser.add_argument("--email", type=str, help="Recipient email address")

    def handle(self, *args, **options):
        tier = options["tier"]
        email = options["email"]
        print(
            f"Scheduling to generate Monthly Storage Active Usage Report to {email}..."
        )
        schedule(
            func="coldfront.plugins.integratedbilling.management.commands.add_schedule_to_generate_monthly_storage_usage_report.generate_monthly_storage_usage_report",
            name="Generate Monthly Storage Active Usage Report",
            schedule_type=Schedule.MONTHLY,
            next_run=SCHEDULED_FOR_2ND_DAY_OF_MONTH_AT_6_AM,
            email=email,
        )


def generate_monthly_storage_usage_report(
    usage_date=datetime.now(timezone.utc).strftime("%Y-%m-01"),
    email: str = None,
    **kwargs,
):
    """
    Generate the Monthly Storage Active Usage Report and email it.
    """
    print(f"Generating Monthly Storage Active Usage Report for emailing to {email}.")

    report_agent = StorageUsageReport(
        usage_date=usage_date, tier=kwargs.get("tier", "1")
    )
    usage_report = report_agent.generate_report()
    print(usage_report)

    subject = f"Monthly Storage Active Usage Report with consumptions on {usage_date}"
    message = f"Here is the Monthly Storage Active Usage report with consumptions on {usage_date} by department per PIs:\n\n{usage_report}"
    from_email = "noreply@gowustl.onmicrosoft.com"
    recipient_list = [email] if email else []

    if recipient_list:
        send_mail(
            subject,
            message,
            from_email,
            recipient_list,
            fail_silently=False,
        )
        print(
            f"Monthly Storage Active Usage Report with consumptions on {usage_date} has been sent via email to {email}."
        )
    else:
        print("No recipient email provided. Report not sent.")
