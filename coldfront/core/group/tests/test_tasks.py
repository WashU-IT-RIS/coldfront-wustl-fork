...
class GrantUserSupportGlobalProjectManagerTest(TestCase):
    def setUp(self):
        ...
        user1 = User.objects.get_or_create(username="user1", id=1).first()
        user2 = User.objects.get_or_create(username="user2", id=2).first()
        self.user1 = user1
        self.user2 = user2
        # Create a user that is not a PI
        user3 = User.objects.get_or_create(username="user3", id=3).first()
        self.user3 = user3
        ...
        self.role_choice = ProjectUserRoleChoice.objects.get_or_create(
            name="Manager"
        ).first()
