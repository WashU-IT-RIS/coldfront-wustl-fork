...
class ProjectListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        form_data_present = any(
            self.request.GET.get(field, "").strip()
...
class ProjectUpdateView(
    ...
    def form_valid(self, form):
        sponsor_department_attribute = (
            self.get_object()
            .projectattribute_set.filter(
                proj_attr_type__name="sponsor_department_number"
            )
            .first()
        )
        sponsor_department_attribute.value = form.cleaned_data.get(
    ...
    def get_form_kwargs(self):
        ...
        sponsor_department_attribute = self.object.projectattribute_set.filter(
            proj_attr_type__name="sponsor_department_number"
        ).first()
        kwargs["initial"]["sponsor_department_number"] = (
            sponsor_department_attribute.value if sponsor_department_attribute else ""
        )
