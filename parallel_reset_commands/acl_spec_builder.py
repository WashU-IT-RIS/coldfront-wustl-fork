...
class ACL_SpecBuilder:
    ...
    def build_specs(
        ...
        with open(
            ...
            for sub_alloc_name in sub_alloc_names:
                self.sub_alloc_root_specs[sub_alloc_name] = (
                    sub_alloc_root_template.replace("<SUB_ALLOC>", sub_alloc_name)
                    .replace("<ALLOC>", alloc_name)
                    .replace("<STORAGE_SUFFIX>", storage_suffix)
                )
        with open(
            ...
            for sub_alloc_name in sub_alloc_names:
                self.sub_alloc_folder_specs[sub_alloc_name] = (
                    sub_alloc_folder_template.replace("<SUB_ALLOC>", sub_alloc_name)
                    .replace("<ALLOC>", alloc_name)
                    .replace("<STORAGE_SUFFIX>", storage_suffix)
                )
        with open(
            ...
            for sub_alloc_name in sub_alloc_names:
                self.sub_alloc_file_specs[sub_alloc_name] = (
                    sub_alloc_file_template.replace("<SUB_ALLOC>", sub_alloc_name)
                    .replace("<ALLOC>", alloc_name)
                    .replace("<STORAGE_SUFFIX>", storage_suffix)
                )
    ...
    def get_spec_by_path(self, target: str, item_type: str):
        ...
        for sub_alloc_root, sub_alloc_name in zip(
            sub_alloc_roots, self.sub_alloc_names
        ):
